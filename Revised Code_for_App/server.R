...
server <- function(input, output) {
      ...
      if (N == 1) {    
        ...
        out.sum = out %>% as_data_frame %>%
          ...
          dplyr::summarise(Blood.5   = (quantile(Blood, 0.025)/VB),
                           ...
                           Liver.5   = (quantile(Liver, 0.025)/VL),  
                           Liver.25  = (quantile(Liver, 0.25)/VL),  
                           Liver.50  = (quantile(Liver, 0.5)/VL),     
                           Liver.75  = (quantile(Liver, 0.75)/VL),     
                           Liver.95  = (quantile(Liver, 0.975)/VL), 
                           Kidney.5  = (quantile(Kidney, 0.025)/VK), 
  ...
  r2 <- reactive({
    ...
    datatable <- insertRows(datatable, 1, new = c("Unit", rep("%ID", 44)))
    return(datatable)
  })
